...
func (Docker) Up() error {
	...
	return sh.RunV("docker-compose", "-f", "docker-compose.dev.yml", "up", "-d")
}
...
func (Docker) Logs() error {
	...
	return sh.RunV("docker-compose", "-f", "docker-compose.dev.yml", "logs", "-f")
}

// Build builds the Docker image
func (Docker) Build() error {
	fmt.Println("Building Docker image...")
	return sh.RunV("docker-compose", "-f", "docker-compose.dev.yml", "build")
}

// Rebuild rebuilds Docker images without cache
func (p Docker) Rebuild() error {
	fmt.Println("Rebuilding Docker images (no cache)...")
	return sh.RunV("docker-compose", "-f", "docker-compose.dev.yml", "build", "--no-cache")
}
